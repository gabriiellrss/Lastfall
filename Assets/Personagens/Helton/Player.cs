...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Guarda a posição inicial para o renascimento
    ...
    void Update()
    {
        // --- MODIFICADO: Não executa Update se estiver morto ---
        if (isDead) return;
        // ------------------------------------------------------
    ...
    void LateUpdate()
    {
        // --- MODIFICADO: Não executa LateUpdate se estiver morto ---
        if (isDead) return;
        // ---------------------------------------------------------
    ...
    IEnumerator ResetAttackState(float delay, bool isCombo1)
    {
        ...
        if (isCombo1)
        {
            if (comboTimer1 > 0) currentCombo1 = (currentCombo1 + 1) % 4;
            else currentCombo1 = 0;
        }
        else
        {
            if (comboTimer2 > 0) currentCombo2 = (currentCombo2 + 1) % 2;
            else currentCombo2 = 0;
        }
    }
    ...
    public void TakeDamage(float damageAmount)
    {
        ...
        // Verifica morte (e se já não está morto)
    ...
    private void Die()
    {
        ...
        // Opcional: Desativar componentes visuais (renderizadores)
        // foreach (Renderer rend in GetComponentsInChildren<Renderer>())
        // { rend.enabled = false; }

        // Inicia a rotina de renascimento
        StartCoroutine(RespawnCoroutine());

        // NÃO Destrói mais o objeto: Destroy(gameObject);
    }
    // -----------------------------

    // --- NOVA COROUTINE PARA RENASCIMENTO ---
